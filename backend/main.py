...
if __name__ == "__main__": 
    count = 0
    for player in get_all_players_in_division(Queue.RANKED_SOLO_5x5, Tier.I, Division.DIAMOND):
        count += 1
        summoner = player.summonerName

        puuid = _get_puuid_by_name(summoner)
        if puuid is None:
            continue
        ...
        print('Grabbing games for {} (#{}), in {} {}'.format(summoner, count, Division.PLATINUM, Tier.I))
        # Grab all their games
        match_ids = list(get_all_match_ids_for_puuid(puuid, QueueType.Ranked5v5))
        ...
        with Pool(num_processes) as pool:
            for match_results in pool.imap_unordered(work, new_match_ids, max(1, int(lnmi / num_processes))):
                if match_results is not None:
